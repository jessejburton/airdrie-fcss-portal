...
<!--- BEGIN MESSAGES --->
			...
			<cfif isDefined('PROGRAM')>
				...
				<cfif PROGRAM.Status IS 'LOI - Approved'>
					<cflocation url="application_form.cfm" addtoken="false">
				</cfif>
			</cfif>
...
<!--- BEGIN FORM --->	
		<cfif showForm>
			<form id="letter_of_intent_form">
				...
				<p>
					<a href="javascript:;" id="save" class="save btn btn-primary inline <cfoutput>#iif(isDefined('PROGRAM'), DE(''), DE('disabled'))#</cfoutput>"><i class="fa fa-save"></i> Save</a>
